...
import {
  ...
  FileText, Image, Video, Mic, File, ArrowLeft, Rocket, X, Sparkles, FlaskConical, BarChart2, Brain, Upload, CheckCircle
} from 'lucide-react'
...
function StepEditor({ step, index, total, onUpdate, onDelete, onMoveUp, onMoveDown, onInsertVariable, onOpenStudio }: StepEditorProps) {
  ...
  const acceptAttr = step.type === 'VIDEO' ? 'video/*' : step.type === 'DOCUMENT' ? '*/*' : 'image/*,video/*'

  const uploadFile = async (file: File) => {
    setUploading(true)
    try {
      const res = await campaignsApi.uploadImage(file)
      onUpdate({ mediaUrl: res.data.url, fileName: res.data.name, mimeType: file.type })
    } catch {
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-3 bg-white">
      ...
      {step.type !== 'TEXT' && (
        <div className="mb-2">
          {step.mediaUrl ? (
            <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded px-2 py-1.5 text-xs text-green-700">
              <CheckCircle className="h-3.5 w-3.5 shrink-0" />
              <span className="truncate flex-1">{step.fileName || step.mediaUrl.split('/').pop()}</span>
              <button onClick={() => onUpdate({ mediaUrl: undefined, fileName: undefined, mimeType: undefined })} className="text-gray-400 hover:text-red-500">
                <X className="h-3 w-3" />
              </button>
            </div>
          ) : (
            <div
              ...
              className={`flex items-center gap-2 border border-dashed rounded px-2 py-2 cursor-pointer transition-colors text-xs ${isDragging ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-300 text-gray-400 hover:border-gray-400 hover:bg-gray-50'}`}
            >
              {uploading ? (
              ...
              <span>{uploading ? 'Subiendo…' : 'Arrastra o haz clic para subir archivo'}</span>
